...
sequence ::
  ...
  -> f (List a)
sequence xs =
  foldRight (\fa -> \fas -> (:.) <$> fa <*> fas ) (pure Nil) xs
...
filtering p (x:.xs) = (\b -> \ys -> if b then (x:.ys) else ys ) <$> (p x) <*> filtering p xs

--  sequence $ foldRight (\x -> \xs -> map (\b -> if b then (x:.) <$> xs else xs) (p x) ) (pure Nil) as 
